    ...
    private static void validateOpExpression(OpExpression opExpr) throws ValidationException {
        ...
        if (dataTypeLeft != null && dataTypeRight != null) {
            ...
            if (!ExprUtil.arrayContains(dataTypes, dataTypeLeft)) {
                throw new ValidationException("Type Error: Expected " + getExpectedTypes(dataTypes), BahnexprPackage.Literals.OP_EXPRESSION__LEFT_EXPR);
            }

            if (!ExprUtil.arrayContains(dataTypes, dataTypeRight)) {
                throw new ValidationException("Type Error: Expected " + getExpectedTypes(dataTypes), BahnexprPackage.Literals.OP_EXPRESSION__RIGHT_EXPR);
            }

            if (!Objects.equals(dataTypeLeft, dataTypeRight)) {
                throw new ValidationException("Type Error: Expressions must have the same type. Actual: " + dataTypeLeft.displayTypeName() + " and " + dataTypeRight.displayTypeName(), BahnexprPackage.Literals.OP_EXPRESSION__RIGHT_EXPR);
            }
        }
    }
