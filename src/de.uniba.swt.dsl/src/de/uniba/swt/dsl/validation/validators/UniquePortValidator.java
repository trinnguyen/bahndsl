...
public class UniquePortValidator {
    private final Map<String, Set<Long>> mapBoardPort = new HashMap<>();
    ...
    public <T> List<Tuple<String, Integer>> validateUniqueAddress(String boardName, List<T> items, Function<T, String> addrMapper) {
        ...
        if (!mapBoardPort.containsKey(boardName)) {
            mapBoardPort.put(boardName, new HashSet<>());
        }

        var set = mapBoardPort.get(boardName);
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            // verify duplication
            if (set.contains(val)) {
                errors.add(Tuple.of(String.format(ValidationErrors.DefinedPortFormat, strVal, boardName), i));
            } else {
                set.add(val);
            }
        }
